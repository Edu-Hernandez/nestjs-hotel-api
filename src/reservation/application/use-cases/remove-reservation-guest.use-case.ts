...
@Injectable()
export class RemoveReservationGuestUseCase {
  constructor(
  ...
  async execute(
    ...
    const removed = await this.reservationRepository.deleteReservationGuest(
      reservationId,
      propertyId,
      guestId,
    );
    if (removed === 0) {
      throw new NotFoundException(
        `Guest ${guestId} is not linked to this reservation`,
      );
    }
  }
}
